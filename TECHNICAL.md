...
| `detect_drawing_frames` の `min_side=400` 固定閾値 | 図面枠の縦辺の高さがこれ未満だと枠が見つからない。実例: `EE6097-039-06C.dxf`（高さ277×幅385の枠、INSERT `#E02`/`#E03`/`#E04` 内の lineweight=100 LINE 4本——INSERT展開自体は正しく動作している）。`DXF-extract-labels` と共有するアルゴリズムなので、閾値を下げる場合は両プロジェクトの全サンプルで退行確認が必要（未着手・報告のみ）。 |
...
*最終更新: 2026-06-24（Consolidate Layers が領域線種の LWPOLYLINE を LINE/ARC に分解して
